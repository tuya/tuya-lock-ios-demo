...
@property (strong, nonatomic) NSArray<TuyaSmartLockRecordModel *> *alermList;
@property (strong, nonatomic) NSArray<TuyaSmartBLELockRecordModel *> *unlockRecordList;
...
- (void)reloadData{
    ...
    if (self.logType == 1){
        [self.bleDevice getAlarmRecordListWithOffset:0
                                               limit:50
                                             success:^(NSArray<TuyaSmartLockRecordModel *> * _Nonnull records) {
            weakSelf_TYSDK.alermList = records;
            [weakSelf_TYSDK.tableView reloadData];
        ...
        [self.bleDevice getUnlockRecordListWithOffset:0
                                                limit:50
                                              success:^(NSArray<TuyaSmartBLELockRecordModel *> * _Nonnull records) {
            weakSelf_TYSDK.unlockRecordList = records;
            [weakSelf_TYSDK.tableView reloadData];
        } failure:^(NSError *error) {
            [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"获取列表失败" message:error.localizedDescription];
        }];
        ...
        [self.bleDevice getProUnlockRecordListWithDevId:self.bleDevice.deviceModel.devId
                                    ...
                                    onlyShowMediaRecord:NO
                                             ...
                                             lastRowKey:@""
                                                  limit:50
                                                success:^(id result) {
            weakSelf_TYSDK.proDataList = result[@"records"];
            [weakSelf_TYSDK.tableView reloadData];
        } failure:^(NSError *error) {
            [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"获取列表失败" message:error.localizedDescription];
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.logType == 1){
        TuyaSmartLockRecordModel *alermModel = self.alermList[indexPath.row];
        ...
        TuyaSmartBLELockRecordModel *unlockRecordModel = self.unlockRecordList[indexPath.row];
    ...
    [cell reloadData:titleStr time:time];
    return cell;
}
...
- (UIAlertController *)memberAlert{
    ...
    [home getHomeMemberListWithSuccess:^(NSArray<TuyaSmartHomeMemberModel *> *memberList) {
        for (int i = 0; i< memberList.count; i++) {
            TuyaSmartHomeMemberModel *member = [memberList objectAtIndex:i];
        ...
        [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"获取成员列表失败" message:error.localizedDescription];
    }];
