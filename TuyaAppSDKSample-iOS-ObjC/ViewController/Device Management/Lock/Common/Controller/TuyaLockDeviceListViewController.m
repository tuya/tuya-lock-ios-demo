...
@interface TuyaLockDeviceListViewController ()<TuyaSmartHomeDelegate,UITableViewDelegate,UITableViewDataSource,TuyaSmartBLELockDeviceDelegate>

@property (strong, nonatomic) TuyaSmartHome *home;
@property (strong, nonatomic) UITableView *tableView;
@property (strong, nonatomic) NSArray *datalist;///tableview数据源
@property (strong, nonatomic) NSArray<TuyaSmartBLELockMemberModel *> *memberList;
...
- (void)refreshData{
    ...
    [self.bleDevice publishSyncBatchDataOfActiveTimeSuccess:^{
        NSLog(@"T0数据下发成功");
    } failure:^(NSError *error) {
        NSLog(@"T0数据下发失败");
    }];
    ...
    switch (indexPath.row) {
        ...
        case 6:
        {
            if (![self.bleDevice isBLEConnected]){
                [[TuyaSmartBLEManager sharedInstance] connectBLEWithUUID:self.bleDevice.deviceModel.uuid
