...
- (void)updateMemberAction:(TuyaSmartHomeMemberRequestModel *)model{
    WEAKSELF_TYSDK
    [self.bleDevice updateProLockMemberInfoWithRequestModel:model success:^{
        ...
        [weakSelf_TYSDK.navigationController popViewControllerAnimated:YES];
    } failure:^(NSError *error) {
        [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"更新家庭成员失败" message:error.localizedDescription];
    }];
}

- (void)addMemberAction:(TuyaSmartHomeAddMemberRequestModel *)model{
    WEAKSELF_TYSDK
    ...
    [self.bleDevice createProLockMemberWithHomeId:homeId
        ...
        [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"创建家庭成员失败" message:error.localizedDescription];
    }];
}
