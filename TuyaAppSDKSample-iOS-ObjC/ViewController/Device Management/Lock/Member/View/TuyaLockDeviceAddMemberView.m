...
- (void)setupViews{
    ...
    [self.roleView bk_whenTapped:^{
        if (weakSelf_TYSDK.delegate && [weakSelf_TYSDK.delegate respondsToSelector:@selector(selectRoleType)]){
            [weakSelf_TYSDK.delegate selectRoleType];
        }
    }];
...
- (void)saveBtnClicked{
    if (self.isEdit){
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(updateMemberAction:)]){
            TuyaSmartHomeMemberRequestModel *model = [[TuyaSmartHomeMemberRequestModel alloc] init];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(addMemberAction:)]){
            TuyaSmartHomeAddMemberRequestModel *model = [[TuyaSmartHomeAddMemberRequestModel alloc] init];
            model.name = self.nameView.contentTextField.text;
