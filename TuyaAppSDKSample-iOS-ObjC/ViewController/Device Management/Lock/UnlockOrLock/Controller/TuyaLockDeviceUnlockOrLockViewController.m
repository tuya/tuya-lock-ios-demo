...
@interface TuyaLockDeviceUnlockOrLockViewController ()<UITableViewDelegate,UITableViewDataSource,TuyaSmartBLELockDeviceDelegate>
@property (strong, nonatomic) UITableView *tableView;
@property (strong, nonatomic) NSArray *datalist;
@property (strong, nonatomic) UILabel *resultLabel;//操作成功、操作失败、失败原因提示
@property (strong, nonatomic) TuyaSmartBLELockDevice *bleLockDevice;
...
- (void)viewDidLoad {
    ...
    self.bleDevice = [[TuyaSmartBLELockDevice alloc] initWithDeviceId:self.devId];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        case 0: {//蓝牙开锁
            [self.bleLockDevice bleCurrentMemberDetailWithDevId:self.devId gid:self.bleLockDevice.deviceModel.homeId success:^(NSDictionary *dict) {
                NSString *bleUnlock = [dict objectForKey:@"lockUserId"];
                [weakSelf_TYSDK.bleLockDevice bleUnlock:bleUnlock success:^{
                    NSLog(@"bleLockDevice success!");
                    [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"开锁成功" message:@""];
        ...
        case 4:
        {
            if (![self.bleLockDevice isBLEConnected]){
                [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"连接失败" message:@""];
            }else{
                [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"蓝牙已连接" message:@""];
            }
        }
            break;
        case 5:
                ...
                if (activeTimeDic) {
                    ...
                    if (distributed) {
                        [self.bleDevice publishDps:@{dpId : dpValue} success:^{
                            [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"同步数据下发成功" message:@""];
                        } failure:^(NSError *error) {
                            [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"同步数据下发失败" message:@""];
                        }];
                    }
                    else{
                        [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"无需同步下发数据" message:@""];
                    }
                }
            } failure:^(NSError *error) {
                [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"同步数据接口请求失败" message:error.localizedDescription];
            }];
        }
        ...
        {   
            [self.bleDevice getLockDeviceConfigWithProductId:self.bleLockDevice.deviceModel.productId
                                                     ...
                                                     success:^(id result) {
                [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"接口请求成功" message:@""];
            } failure:^(NSError *error) {
                [Alert showBasicAlertOnVC:weakSelf_TYSDK withTitle:@"配置接口请求失败" message:error.localizedDescription];
            }];
...
- (TuyaSmartBLELockDevice *)bleLockDevice{
    if (!_bleLockDevice){
        _bleLockDevice = [[TuyaSmartBLELockDevice alloc] initWithDeviceId:self.devId];
    }
